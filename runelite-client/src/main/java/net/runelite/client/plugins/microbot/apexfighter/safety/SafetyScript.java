package net.runelite.client.plugins.microbot.apexfighter.safety;
...
public class SafetyScript extends Script {
    public boolean run(ApexFighterConfig config) {
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (config.missingArrows() && !(Rs2Equipment.isWearing("arrow") || Rs2Equipment.isWearing("bolt") || Rs2Equipment.isWearing("dart") || Rs2Equipment.isWearing("knife"))) {
    ...
    public void stopAndLog(String reason) {
        ...
        if(Rs2Bank.walkToBank()){
            Rs2Player.logout();
            Plugin PlayerAssistPlugin = Microbot.getPlugin(ApexFighterPlugin.class.getName());
